	...
	<s2ui:form width='650' height='300' elementId='loginFormContainer'
		...
		<g:form action='register' name='registerForm'>
			...
			<g:else>
				...
				<table>
					<tbody>
						<g:hiddenField name="username" id="user_name"
							bean="${command}" value="${command.username}" />
<%--								<s2ui:textFieldRow name='username' labelCode='user.username.label' bean="${command}"--%>
<%--						                         size='40' labelCodeDefault='Username' value="${command.username}"/>--%>

						<s2ui:textFieldRow name='email' id="user_emial" type="email" bean="${command}"
				...
				<s2ui:submitButton elementId='create' form='registerForm'
					messageCode='spring.security.ui.register.submit' onclick="onTextEnter(event)"/>
